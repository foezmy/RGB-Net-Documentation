...
class PairedDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.training and self.crop_size:
            ...
            if low_image.shape[1] < self.crop_size or low_image.shape[2] < self.crop_size:
                # 尺寸不足时缩放至crop_size
                low_image = F.interpolate(low_image.unsqueeze(0), size=(self.crop_size, self.crop_size),
                                          mode='bilinear').squeeze(0)
                high_image = F.interpolate(high_image.unsqueeze(0), size=(self.crop_size, self.crop_size),
                                           mode='bilinear').squeeze(0)
            else:
